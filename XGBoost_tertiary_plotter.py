...
def load_additional_data(csv_file, target_dates):
    ...
    target_dates_as_date = [
        datetime.datetime.fromtimestamp(ts).date() for ts in target_dates
    ]
...
def filter_data_for_ids(data, num_ids=2):
    ...
    filtered_sequences, filtered_targets, filtered_dates, target_dates, brent = (
        [],
        [],
        [],
        [],
...
def flatten_input(sequences, dates, brent):
    """
    ...
    combined_input = np.stack([sequences, dates], axis=-1).reshape(-1, 14)
...
def plot_predictions(id_data):
    ...
    base_filename = "./Plots/XGB_Tertiary_Model"
    extension = ".png"
    counter = 0
